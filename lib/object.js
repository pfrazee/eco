...
module.exports = function(db, feed, state) {
  ...
  obj.applyMessage = function(msg, opts, cb) {
    ...
    var vts = (opts.nostamp) ? msgData.vts : stamp() // note, opts.nostamp means that this message was just generated internally, so it's fine to use its vts

    if (msgData.path === '') {
      // operation on the object
      var result = apply(msgData, msg.author, authi, vts)
      ...
      if (result) {
        ...
        return persist(function(err) {
          if (err) return cb(err)
          cb(false, result[0], result[1], result[2], { author: msg.author, authi: authi, vts: vts })
        })
      }
      return cb()
    }
    ...
    var result = types[meta.type].apply(state, msgData, authi)
    if (result) {
      if (result instanceof Error)
        return cb(result)

      // update vts
      state.meta[msgData.path].vts = vts

      // store new state
      persist(function(err) {
        if (err) return cb(err)
        cb(false, msgData.path, result[0], result[1], { author: msg.author, authi: authi, vts: vts })
      })
      if (Array.isArray(result)) // a visible change? (as signified by a diff array)
        obj.emit('change', msgData.path, result[0], result[1], { author: msg.author, authi: authi, vts: vts })
    } else
      cb()
  }
  ...
  function publish(msg, cb) {
    ...
    feed.add('eco', msgpack.encode(msg), function(err, msg) {
      obj.applyMessage(msg, {nostamp: true}, cb)
    })
  }
