...
exports.apply = function(state, msg, authi) {
  ...
  if (msg.op == 'set') {
    ...
    if (!meta.added[key].length) {
      ...
      meta.authis[key] = authi // track the author index
      return [[key, old], [key, v]]
    }

    // is this tag the one we've already seen?
    if (meta.added[key].indexOf(addTag) === -1) {
      // no, so there's been a concurrent change - take greatest authority
      meta.added[key].push(addTag) // track the tag no matter what (it's been observed)
      if (typeof meta.authis[key] != 'undefined' && meta.authis[key] < authi)
        return true // existing authority is greater, abort but persist meta updates
      if (meta.vts[authi] >= msg.vts[authi])
        return true // we have more recent messages from this author, abort but persist meta updates [1]
