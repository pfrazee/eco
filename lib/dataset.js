...
exports.dataset = function(ssb, feed, opts) {
  ...
  // Define methods immutably so that declared sub-objects dont overwrite
  function method(name, def) {
    Object.defineProperty(dataset, name, { value: def })
  }
  ...
  // Declare operation
  method('declare', function(key, def, cb) {
    if (key && typeof key == 'object') {
      for (var k in key)
        this.declare(k, key[k], cb||def)
      return
    }
    ...
    // :TODO: make sure the object doesnt exist yet

    queue(msglib.create('', 'declare', key, def), cb) // :TODO: path
  })

  // Remove operation
  method('remove', function(key, cb) {
    if (!key || typeof key != 'string')
      throw new Error('`key` is required and must be a string')
