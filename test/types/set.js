...
module.exports = function(opts) {
  tape('set - one member', function(t) {
    var db = tutil.makedb()
    ...
    eco.create(db, feed, function(err, obj) {
      ...
      // declare
      console.log('obj1 declaring orset')
      obj.declare({ orset: 'set' }, function(err, changes) {
        if (err) throw err

        console.log('changes', changes)
        t.equal(feed.msgs.length, 2)
        t.equal(changes.length, 1)
        t.assert(equal(obj.get().orset, []))

        // add 'a', 1, true
        console.log('obj1 setting orset to ["a", 1, true]')
        obj.put({ orset: ['a', 1, true] }, function(err, changes) {
          if (err) throw err
          ...
          t.equal(changes[0][0], 'orset')
          t.equal(changes[0][1], void 0)
          t.equal(changes[0][2], 'a')
          t.equal(changes[1][0], 'orset')
          t.equal(changes[1][1], void 0)
          t.equal(changes[1][2], 1)
          t.equal(changes[2][0], 'orset')
          t.equal(changes[2][1], void 0)
          t.equal(changes[2][2], true)
          t.equal(umsg1.path, 'orset')
          t.equal(umsg1.op, 'add')
          t.equal(umsg1.args[0], 'a')
          t.equal(umsg2.path, 'orset')
          t.equal(umsg2.op, 'add')
          t.equal(umsg2.args[0], 1)
          t.equal(umsg3.path, 'orset')
          t.equal(umsg3.op, 'add')
          t.equal(umsg3.args[0], true)
          t.assert(equal(obj.get().orset, ['a', 1, true]))

          // add 0, 'aa', remove 1
          console.log('obj1 setting orset to ["a", true, "aa", 0]')
          obj.put({ orset: ['a', true, 'aa', 0] }, function(err, changes) { // note, the 1 value is not included
            if (err) throw err
            ...
            t.equal(changes[2][0], 'orset')
            t.equal(changes[2][1], 1)
            t.equal(changes[2][2], undefined)
            t.equal(umsg1.path, 'orset')
            t.equal(umsg1.op, 'add')
            ...
            t.equal(umsg3.path, 'orset')
            t.equal(umsg3.op, 'remove')
            t.equal(umsg3.args[0], 1)
            t.assert(equal(obj.get().orset, ['a', true, 'aa', 0]))

            // re-add 1
            console.log('obj1 setting orset to ["a", true, "aa", 0, 1]')
            obj.put({ orset: ['a', true, 'aa', 0, 1] }, function(err, changes) {
              if (err) throw err
              t.equal(changes.length, 1)
              t.assert(equal(obj.get().orset, ['a', true, 'aa', 0, 1]))
              db.close(t.end)
            })
          })
        })        
  ...
  tape('set - two members, updated concurrently by both', function(t) {
    ...
    eco.create(db1, feed1, {members:[feed1.id, feed2.id]}, function(err, obj1) {
      ...
      // declare
      console.log('obj1 declaring orset')
      obj1.declare({ orset: 'set' }, function(err, changes) {
        if (err) throw err

        console.log('feed1 changes', changes)
        t.equal(feed1.msgs.length, 2)
        t.equal(changes.length, 1)
        t.assert(equal(obj1.get().orset, []))

        // set to ['a', 1, true]
        console.log('obj1 setting orset to ["a", 1, true]')
        obj1.put({ orset: ['a', 1, true] }, function(err, changes) {
          if (err) throw err
          ...
          console.log('obj1', obj1.get().orset)
          console.log('changes', changes)
          console.log('update message 1', umsg1)
          console.log('update message 2', umsg2)
          console.log('update message 3', umsg3)
          ...
          t.equal(changes.length, 3)
          t.assert(equal(obj1.get().orset, ['a', 1, true]))
          ...
          eco.open(db2, feed2, obj1.getId(), function(err, obj2) {
            ...
            obj2.applyMessages(feed2.msgs.slice(1), function(err, changes) {
              ...
              t.assert(equal(obj2.get().orset, ['a', 1, true]))

              // add false, 2, 3
              console.log('obj2 setting orset to ["a", 1, true, false, 2, 3]')
              obj2.put({ orset: ['a', 1, true, false, 2, 3] }, function(err, changes) {
                if (err) throw err
                ...
                t.equal(changes.length, 3)
                t.assert(equal(obj2.get().orset, ['a', 1, true, false, 2, 3]))

                // add 2, 4, remove true
                console.log('obj1 setting orset to ["a", 1, 2, 4]')
                obj1.put({ orset: ['a', 1, 2, 4] }, function(err, changes) {
                  if (err) throw err
                  ...
                  t.equal(changes.length, 3)
                  t.assert(equal(obj1.get().orset, ['a', 1, 2, 4]))
                  ...
                  obj1.applyMessages(feed1.msgs.slice(8), function(err, changes) {
                    ...
                    t.assert(equal(obj1.get().orset, ['a', 1, 2, 4, false, 3]))
                    ...
                    obj2.applyMessages(feed2.msgs.slice(8), function(err, changes) {
                      ...
                      t.equal(changes.length, 2)
                      t.assert(equal(obj2.get().orset, ['a', 1, false, 2, 3, 4]))

                      // remove 2
                      console.log('obj1 setting orset to ["a", 1, 4, false, 3]')
                      obj1.put({ orset: ['a', 1, 4, false, 3] }, function(err, changes) {
                        if (err) throw err

                        console.log('obj1', obj1.get())
                        console.log('feed1 changes', changes)
                        t.equal(feed1.msgs.length, 12)
                        t.equal(changes.length, 1)
                        t.assert(equal(obj1.get().orset, ['a', 1, 4, false, 3]))
                        ...
                        // bring feed2 history up to date
                        obj2.applyMessages(feed2.msgs.slice(11), function(err, changes) {
                          if (err) throw err

                          console.log('obj2', obj2.get())
                          console.log('feed2 changes', changes)
                          t.equal(feed2.msgs.length, 12)
                          t.equal(changes.length, 1)
                          t.assert(equal(obj2.get().orset, ['a', 1, false, 3, 4]))
