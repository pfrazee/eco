  ...
  tape('object open from other feed', function(t) {
    ...
    eco.create(db1, feed1, { members: [feed1.id, feed2.id] }, function(err, obj1) {
      ...
      eco.open(db2, feed2, obj1.getId(), function(err, obj2) {
        ...
        console.log('seq', state2.seq)
        t.assert(equal(state1.seq, 1))
        t.assert(equal(state2.seq, 1))
  ...
  tape('value declarations', function(t) {
    ...
    eco.create(db, feed, function(err, obj) {
      ...
      obj.declare({
        ...
        t.assert(equal(state.meta.a.seq, 2))
        t.assert(equal(state.meta.b.seq, 3))
        t.equal(state.meta.b.type, 'counterset')
        t.assert(equal(state.meta.c.seq, 4))
        t.equal(state.meta.c.type, 'growset')
        t.assert(equal(state.meta.d.seq, 5))
        t.equal(state.meta.d.type, 'map')
        t.assert(equal(state.meta.e.seq, 6))
        t.equal(state.meta.e.type, 'onceset')
        t.assert(equal(state.meta.f.seq, 7))
        t.equal(state.meta.f.type, 'register')
        t.assert(equal(state.meta.g.seq, 8))
      ...
      obj1.declare({
        ...
        eco.open(db2, feed2, obj1.getId(), function(err, obj2) {
          ...
          obj2.applyMessages(feed2.msgs.slice(1), function(err, changes) {
            ...
            t.assert(equal(state2.meta.a.seq, 2))
            t.assert(equal(state2.meta.b.seq, 3))
            t.assert(equal(state2.meta.c.seq, 4))
            t.assert(equal(state2.meta.d.seq, 5))
            t.assert(equal(state2.meta.e.seq, 6))
            t.assert(equal(state2.meta.f.seq, 7))
            t.assert(equal(state2.meta.g.seq, 8))
