  ...
  tape('object open from other feed', function(t) {
    ...
    eco.create(db1, feed1, { members: [feed1.id, feed2.id] }, function(err, obj1) {
      ...
      eco.open(db2, feed2, obj1.getId(), function(err, obj2) {
        ...
        t.assert(equal(state2.vclock, [1, 0]))
      ...
      obj1.declare({
        ...
        eco.open(db2, feed2, obj1.getId(), function(err, obj2) {
          ...
          obj2.applyMessages(feed2.msgs.slice(1), function(err, changes) {
            ...
            t.assert(equal(state2.meta.a.vts, [2, 0]))
            t.assert(equal(state2.meta.b.vts, [3, 0]))
            t.assert(equal(state2.meta.c.vts, [4, 0]))
            t.assert(equal(state2.meta.d.vts, [5, 0]))
            t.assert(equal(state2.meta.e.vts, [6, 0]))
            t.assert(equal(state2.meta.f.vts, [7, 0]))
            t.assert(equal(state2.meta.g.vts, [8, 0]))
